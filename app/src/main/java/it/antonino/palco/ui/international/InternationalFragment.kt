...
class InternationalFragment: Fragment() {
    ...
    private val cityObserver = Observer<ArrayList<City?>?> {
        when (!it.isNullOrEmpty()) {
            true -> {
                for (city in it) {
                    if (city?.getCity()?.last()?.isUpperCase() == true)
                        cityList.add(city.getCity().substring(0, city.getCity().length - 2))
                    else
                ...
                cityAdapter = CityListAdapter(cityList) {
                    ...
                    viewModel.getConcertiInternazionaliByCity(it).observe(viewLifecycleOwner, concertiObserver)
                }
